...
describe('Home', () => {
  ...
  it('should verify title, subtitle and link from create account grid', () => {
    ...
    expect(wrapper.find(selectors.createAccountTitle).text()).toEqual('Bem-Vindo');
    expect(wrapper.find(selectors.createAccountSubtitle).text()).toContain('Novo aqui?');
    expect(wrapper.find(selectors.createAccountLink).text()).toEqual('Criar uma conta');
    expect(wrapper.find(selectors.createAccountLink).attributes().href).toEqual('/create-account')
  });

  it('should verify texts of toast error', () => {
    const wrapper = shallowMount(Home);
    expect(wrapper.find(selectors.toastError).text()).toEqual('Usuário ou senha Inválido');
  });
